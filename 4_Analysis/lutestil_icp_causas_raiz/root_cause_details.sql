-- Causas raízes das OPs detratoras do ICP — LUTESTIL.
...
WITH params AS (
  SELECT
    DATE '2026-05-18'                                            AS analysis_date,
    DATE_SUB(DATE '2026-05-18', INTERVAL 45 DAY)                AS window_start
),

-- Estado atual de cada OP da LUTESTIL (latest snapshot da tabela denormalizada)
current_sc AS (
  SELECT
    sc.op_code,
  ...
  WHERE REGEXP_CONTAINS(UPPER(COALESCE(sc.supplier_name, '')), r'LUTESTIL')
