...
-- Visão OP-SKU granular por semana: flags INT/EXT sem agregação
SELECT
    w.snapshot_week,
    orig.cycle_name,
    orig.op_code,
    ...
    -- Estado no snapshot da semana
    w.snap.current_production_stage   AS current_stage,
    w.snap.planned_quantity           AS current_planned_qty,
    w.snap.dt_planned_entry_warehouse AS current_dt_planned,
    w.snap.dt_reviewed_entry_warehouse AS current_dt_reviewed,

    -- Flags INT
    w.snap.dt_planned_entry_warehouse != orig.baseline_dt_planned                                              AS is_int_date,
    w.snap.current_production_stage = 'canceled'
        AND canc.canceled_production_reason = 'Revisão de Demanda (In Season)'                                 AS is_int_cancel,
    w.snap.planned_quantity IS NOT NULL
        AND orig.baseline_planned_qty IS NOT NULL
        AND w.snap.planned_quantity != orig.baseline_planned_qty                                               AS is_int_grade,

    -- Flags EXT
    w.snap.current_production_stage = 'canceled'
        AND (canc.canceled_production_reason IS NULL
             OR canc.canceled_production_reason != 'Revisão de Demanda (In Season)')                           AS is_ext_cancel,
    w.snap.dt_reviewed_entry_warehouse IS NOT NULL
        ...
        AND (w.snap.dt_planned_entry_warehouse = orig.baseline_dt_planned
             ...
             OR orig.baseline_dt_planned IS NULL)                                                              AS is_ext_date_rev,

    -- Deltas em dias (útil para ver magnitude da mudança)
    DATE_DIFF(w.snap.dt_planned_entry_warehouse, orig.baseline_dt_planned, DAY)    AS delta_days_planned,
    DATE_DIFF(w.snap.dt_reviewed_entry_warehouse, orig.baseline_dt_reviewed, DAY)  AS delta_days_reviewed
...
WHERE orig.cycle_name = 'C062026'
ORDER BY orig.op_code, orig.product_sku, w.snapshot_week