-- Detalhe das OPs Fitmax apontadas na analise de ICP (coorte de entrada 2026-06-01):
-- OPF45N513: Shorts Esportivo Serotonin Feminino (702 pecas) — prazo 15/05, 16 dias de atraso
-- OPF54N81:  Performance T-shirt 2.0 Masculino (928 pecas)   — prazo 25/05
...
WITH current_sc AS (
  ...
  WHERE sc.op_code IN ('OPF45N513', 'OPF54N81')
...
quality AS (
  ...
  WHERE op_code IN ('OPF45N513', 'OPF54N81')
)
...
SELECT
  ...
  po.real_fabric_receiving_date > po.expected_fabric_receiving_date
                                                                   AS is_mp_late,
