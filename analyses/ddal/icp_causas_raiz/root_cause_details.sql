-- Causas raízes das OPs detratoras do ICP — DDAL.
...
-- Estado atual de cada OP da DDAL (latest snapshot da tabela denormalizada)
current_sc AS (
  SELECT
    sc.op_code,
    ANY_VALUE(sc.cycle_name)                                     AS cycle_name,
    ANY_VALUE(sc.supplier_name)                                  AS supplier_name,
    ANY_VALUE(sc.nfe_numbers_op)                                 AS nfe_numbers_op,
    ARRAY_TO_STRING(
